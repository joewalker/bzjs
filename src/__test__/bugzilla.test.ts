...
describe('Bugzilla', () => {
  ...
  describe('search', () => {
    ...
    it('should return empty bugs on dryRun without calling fetch', async () => {
      const bz = new Bugzilla({ origin: 'https://bz.test' });
    ...
    it('should return checkUrl alongside bugs', async () => {
      mockFetch({ bugs: [{ id: 1 }] });
      const bz = new Bugzilla({ origin: 'https://bz.test' });
  ...
  describe('count', () => {
    it('should append count_only param and return bugCount', async () => {
      mockFetch({ bug_count: 42 });

      const bz = new Bugzilla({ origin: 'https://bz.test' });
      const result = await bz.count({ product: 'Core' });

      const params = parseQuery(fetchedUrl());
      expect(params).toContainEqual(['product', 'Core']);
      expect(params).toContainEqual(['count_only', 'true']);
      expect(result.bugCount).toBe(42);
    });

    it('should return checkUrl', async () => {
    ...
    it('should return POSITIVE_INFINITY on dryRun without calling fetch', async () => {
      const bz = new Bugzilla({ origin: 'https://bz.test' });
      const result = await bz.count({ dryRun: true });
